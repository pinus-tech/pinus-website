  ...
  try {
    ...
    if (!admin) {
      return NextResponse.json(
        { error: 'Unauthorized. Admin access with user management permission required.' },
        { status: 401 }
      );
    }
...
export async function PATCH(req: NextRequest, { params }: { params: { userId: string } }) {
  try {
    ...
    const { name, email, telegram, phoneNumber, city, major, intakeYear, yearOfStudy, highSchool } = body;
    ...
    if (email && email !== user.email) {
      const existingUser = await User.findOne({ email, _id: { $ne: params.userId } });
      if (existingUser) {
        return NextResponse.json(
